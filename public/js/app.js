            ...
            if(err) console.log(err)
                ...
                for (var i=0; i<=result.transaction_ids.length-1; i++) {
                    if(result.transactions[i].operations[0][0]=="vote"){
                        vote++
                        varia12gt=varia12gt==null ? `<div class="alert alert-primary" role="alert">
                        ${result.transactions[i].operations[0][1].voter} <b> reward </b> ${result.transactions[i].operations[0][1].author} in the post <a href="https://whaleshares.io/@${result.transactions[i].operations[0][1].author}/${result.transactions[i].operations[0][1].permlink}">${(result.transactions[i].operations[0][1].permlink).substring(0,25)}</a> (${(result.transactions[i].operations[0][1].weight)/100}%)
                    </div></ br>` : varia12gt+`<div class="alert alert-primary" role="alert">
                    ...
                    if(result.transactions[i].operations[0][0]=="comment"){
                        comment++
                        var posiblidis=(result.transactions[i].operations[0][1].parent_author=='' ? ' replied to ' : ' authored a post ')
